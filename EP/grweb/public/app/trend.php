<?php
/*
 * JCI-EP-WEB-Patch — native trend-grafiek viewer.
...
class TrendController extends BaseController {
  ...
  public function cptToolsBackupUrl() { return $this->makeUrl("../CPT-Tools.zip"); }
  public function sptToolsBackupUrl() { return $this->makeUrl("../SPT-Tools.zip"); }

  public function plugins() {
    $access_model = new FeatureAccessControl();
    $plugins = collectPlugins();
    foreach($plugins as &$plugin) {
      if (!isset($plugin['indexFile'])) continue;
...
$trendTables = jciTrendListTables();
if ($selectedTable === '' && !empty($trendTables)) $selectedTable = $trendTables[0];
?>
<!DOCTYPE html>
<html>
  <head>
    ...
    <link rel="stylesheet" href="<?php A('../css/apexcharts.css') ?>" />
    <link rel="stylesheet" href="<?php A('../css/apexcharts-legend.css') ?>" />
    ...
    <script language="javascript" type="text/javascript">
      ...
      window.JCI_TREND_TABLE = '<?php echo addslashes($selectedTable) ?>';
      window.JCI_TREND_TABLES = <?php echo json_encode($trendTables) ?>;
    </script>
...
?>
  ...
  <script type="text/javascript" src="<?php A('../js/apexcharts.min.js') ?>"></script>

  </head>
  <body class="theme-h31 jci-trend-page">
    <div id="header_panel">
...
<?php
  ...
  if (!empty($trendTables)) {
    /* This page IS the trend viewer — mark it active + expanded so the
       current selection is highlighted in the sidebar. */
    echo "<li class='parent has-children trendM active expanded'><a href='#' onclick='return false;'><i class='mdi mdi-chart-line'></i><span>".L('Trend')."</span></a>";
    echo "<ul class='sub-menu'><li class='nav-items'><ul class='nav'>";
    foreach ($trendTables as $t) {
      $active = ($t === $selectedTable) ? " class='active'" : '';
      echo "<li" . $active . "><a href='../app/trend.php?table=" . urlencode($t) . "'>" . htmlspecialchars($t, ENT_QUOTES|ENT_HTML401) . "</a></li>";
    }
    echo "</ul></li></ul></li>";
  }
...
?>
    ...
    <!-- Trend page main content -->
    <div id="trendContainer" class="container-fluid">
<?php if (empty($trendTables)) { ?>
      <div class="alarm-card trend-empty-card" style="margin:24px;padding:32px;text-align:center;">
        <i class="mdi mdi-chart-line" style="font-size:48px;color:var(--jci-text-soft)"></i>
        <h3 style="margin-top:16px"><?php echo L('No trend tables') ?></h3>
        <p style="color:var(--jci-text-muted)"><?php echo L('No SQLite tables found at') ?>: <code><?php echo htmlspecialchars(jciTrendDbPath()) ?></code></p>
      </div>
<?php } else { ?>
      <div class="trend-page-tabs">
        <h3 class="trend-page-title" id="trend-title"><?php echo htmlspecialchars($selectedTable) ?></h3>
      </div>
      <div class="trend-layout">
        <!-- Left: points config card -->
        <div class="alarm-card trend-points-card" id="trend-points-card">
          <div class="trend-points-header">
            <span class="trend-points-title"><?php echo L('Points') ?></span>
            <span class="trend-points-actions">
              <button type="button" class="trend-edit-btn" title="<?php echo L('Edit') ?>"><i class="mdi mdi-pencil-outline"></i></button>
              <button type="button" class="trend-save-btn" title="<?php echo L('Save') ?>"><i class="mdi mdi-content-save-outline"></i> <?php echo L('Save') ?></button>
              <button type="button" class="trend-cancel-btn" title="<?php echo L('Cancel') ?>"><?php echo L('Cancel') ?></button>
              <button type="button" class="trend-collapse-btn" id="trend-points-collapse" title="<?php echo L('Collapse') ?>"><i class="mdi mdi-chevron-double-left"></i></button>
            </span>
          </div>
          <div class="trend-points-list" id="trend-points-list">
            <div class="trend-points-empty"><?php echo L('Loading') ?>…</div>
          </div>
        </div>
        <!-- Floating expand handle, only visible when points card is collapsed -->
        <button type="button" class="trend-points-expand" id="trend-points-expand" title="<?php echo L('Show points') ?>"><i class="mdi mdi-chevron-double-right"></i></button>
        <!-- Right: chart card with toolbar -->
        <div class="alarm-card trend-chart-card">
          <div class="trend-chart-toolbar">
            <div class="trend-toolbar-left">
              <button type="button" class="alarm-tbtn alarm-tbtn-icon" id="trend-refresh" title="<?php echo L('Refresh') ?>"><i class="mdi mdi-refresh"></i></button>
              <button type="button" class="alarm-tbtn alarm-tbtn-icon" id="trend-zoom-in" title="<?php echo L('Zoom in') ?>"><i class="mdi mdi-magnify-plus-outline"></i></button>
              <button type="button" class="alarm-tbtn alarm-tbtn-icon" id="trend-zoom-out" title="<?php echo L('Zoom out') ?>"><i class="mdi mdi-magnify-minus-outline"></i></button>
              <button type="button" class="alarm-tbtn alarm-tbtn-icon" id="trend-zoom-reset" title="<?php echo L('Reset zoom') ?>"><i class="mdi mdi-restore"></i></button>
              <button type="button" class="alarm-tbtn alarm-tbtn-icon" id="trend-pan" title="<?php echo L('Pan / select') ?>"><i class="mdi mdi-cursor-move"></i></button>
              <div class="btn-group">
                <button type="button" class="alarm-tbtn alarm-tbtn-export dropdown-toggle" data-toggle="dropdown" id="trend-export-toggle">
                  <i class="mdi mdi-download"></i> <?php echo L('Export') ?> <span class="caret"></span>
                </button>
                <ul class="dropdown-menu" id="trend-export-menu">
                  <li><a href="javascript:void(0)" data-export="png">PNG</a></li>
                  <li><a href="javascript:void(0)" data-export="svg">SVG</a></li>
                  <li><a href="javascript:void(0)" data-export="csv">CSV</a></li>
                </ul>
              </div>
            </div>
            <div class="trend-toolbar-right">
              <div class="btn-group trend-preset-group">
                <button type="button" class="alarm-tbtn dropdown-toggle trend-preset-toggle" data-toggle="dropdown" id="trend-preset-toggle">
                  <i class="mdi mdi-clock-fast"></i> <span class="trend-preset-label"><?php echo L('Quick range') ?></span> <span class="caret"></span>
                </button>
                <ul class="dropdown-menu trend-preset-menu" id="trend-preset-menu">
                  <li><a href="javascript:void(0)" data-preset="1h"><?php echo L('Last hour') ?></a></li>
                  <li><a href="javascript:void(0)" data-preset="6h"><?php echo L('Last 6 hours') ?></a></li>
                  <li><a href="javascript:void(0)" data-preset="24h"><?php echo L('Last day') ?></a></li>
                  <li><a href="javascript:void(0)" data-preset="7d"><?php echo L('Last week') ?></a></li>
                  <li><a href="javascript:void(0)" data-preset="30d"><?php echo L('Last 30 days') ?></a></li>
                  <li class="divider"></li>
                  <li><a href="javascript:void(0)" data-preset="today"><?php echo L('Today') ?></a></li>
                  <li><a href="javascript:void(0)" data-preset="yesterday"><?php echo L('Yesterday') ?></a></li>
                  <li><a href="javascript:void(0)" data-preset="thisweek"><?php echo L('This week') ?></a></li>
                  <li><a href="javascript:void(0)" data-preset="thismonth"><?php echo L('This month') ?></a></li>
                </ul>
              </div>
              <div class="alarm-daterange-pair">
                <div class="input-append date alarm-dt-wrap" data-role="trend-dt-from">
                  <span class="add-on alarm-dt-btn"><i class="mdi mdi-calendar"></i> <span class="alarm-dt-label" data-role="trend-dt-from-label">From</span></span>
                  <input type="text" class="alarm-dt-input alarm-dt-hidden" data-role="trend-date-from" readonly>
                </div>
                <span class="alarm-daterange-sep">→</span>
                <div class="input-append date alarm-dt-wrap" data-role="trend-dt-to">
                  <span class="add-on alarm-dt-btn"><i class="mdi mdi-calendar"></i> <span class="alarm-dt-label" data-role="trend-dt-to-label">To</span></span>
                  <input type="text" class="alarm-dt-input alarm-dt-hidden" data-role="trend-date-to" readonly>
                </div>
              </div>
            </div>
          </div>
          <div class="trend-chart-wrap">
            ...
            <div class="trend-loading" id="trend-loading"><i class="mdi mdi-loading mdi-spin"></i> <?php echo L('Loading') ?>…</div>
            <div class="trend-empty" id="trend-empty" style="display:none"><?php echo L('No data in this period') ?></div>
          </div>
        </div>
      </div>
<?php } ?>
...
?>
    ...
    <!-- Native trend viewer controller. -->
    <script type="text/javascript" src="<?php A('../js/jci-trend.js') ?>"></script>
  </body>
</html>
