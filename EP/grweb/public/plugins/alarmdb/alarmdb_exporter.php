 ...
 */
    ...
    /*
     * Per-user permission gate. Mutating commands are restricted by
     * jci_patch flags in app/grdata/user_perms.json. Admins bypass.
     * Returns true if the command may proceed; otherwise emits 403 +
     * JSON body and returns false.
     */
    protected function jciCanRun($cmdName) {
        $needs = null;
        if ($cmdName === 'ackn')   $needs = 'alarm_ack';
        if ($cmdName === 'delete') $needs = 'alarm_delete';
        if ($needs === null) return true;     // unrestricted command
        $u = $this->curUser();
        if ($u && $u->isAdmin()) return true; // admin bypass
        $userId = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : '';
        if (jciUserCan($userId, $needs)) return true;
        http_response_code(403);
        header('Content-type: application/json;charset=utf-8');
        echo json_encode(array('error' => 'forbidden: ' . $needs . ' permission required'));
        return false;
    }
    ...
    protected function doGet() {
		...
		if (!$this->jciCanRun($get_command)) return;
